...
export default function CamHLSControls(props) {
    const {
        allValues, 
        camera, 
    ...
    const handlePTZ = async({direction})  => {
        ...
        if(direction === 'north'){
            ...
            const apiRes = await moveCamera({streamId: camera?.antStreamId, valueX:'0', valueY:'1.0', valueZ:'0', movement:'relative' })
        }else if(direction === 'northEast'){
            console.log('Move NorthEast')
            const apiRes = await moveCamera({streamId: camera?.antStreamId, valueX:'0.5', valueY:'0.5', valueZ:'0', movement:'relative' })
        }else if(direction === 'east'){
            console.log('Move East')
            const apiRes = await moveCamera({streamId: camera?.antStreamId, valueX:'1', valueY:'0', valueZ:'', movement:'relative' })
        }else if(direction === 'southEast'){
            console.log('Move SouthEast')
            const apiRes = await moveCamera({streamId: camera?.antStreamId, valueX:'0.5', valueY:'-0.5', valueZ:'', movement:'relative' })
        }else if(direction === 'south'){
            console.log('Move South')
            const apiRes = await moveCamera({streamId: camera?.antStreamId, valueX:'0', valueY:'-1.0', valueZ:'', movement:'relative' })
        }else if(direction === 'southWest'){
            console.log('Move SouthWest')
            const apiRes = await moveCamera({streamId: camera?.antStreamId, valueX:'-0.5', valueY:'-0.5', valueZ:'0', movement:'relative' })
        }else if(direction === 'west'){
            console.log('Move West')
            const apiRes = await moveCamera({streamId: camera?.antStreamId, valueX:'-1.0', valueY:'0', valueZ:'0', movement:'relative' })
        }else if (direction === 'northWest'){
            console.log('North West')
            const apiRes = await moveCamera({streamId: camera?.antStreamId, valueX:'-0.5', valueY:'0.5', valueZ:'', movement:'relative' })
        }else if (direction === 'zoomIn'){
            console.log('PTZ Zoom In')
            const apiRes = await moveCamera({streamId: camera?.antStreamId, valueX:'0', valueY:'0', valueZ:'1.0', movement:'relative' })
  ...
  return (
      ...
      <ArchiveVodClipModal open={showArchiveModal} setOpen={setShowArchiveModal} camera={camera}/> */}
        ...
        {/* Top controls */}
        <div
        className="p-1 flex flex-row justify-between items-center absolute top-0 "
        >
        {/* <div >
        ...
        {/* bottom controls */}
        <div
        className="bottom-0 absolute flex flex-col justify-between items-center w-full "
        >
